        ...
        <table class="table table-striped table-bordered">
          <thead>
            <tr class="headings">
              ...
              <!-- <th class="column-title no-link last" style="width: 100px;"></th> -->
            </tr>
          </thead>
          <tbody>
          <?php foreach ($data as $key => $value) {
              ...
              ?>
                    ...
                    <!-- <td>
                      ...
                      <a href="<?=site_url('InvoiceFinance/receive/'.$value['id'])?>" class="btn btn-success btn-sm" onclick="return confirm('Receive Invoice <?=$value['invoice_number']?> ?')"><i class="fa fa-arrow-right"></i> Receive</a> 
                      <?php } ?>
                      <?php if($value['status'] == 2) { ?>
                      <a href="<?=site_url('InvoiceFinance/paid/'.$value['id'])?>" class="btn btn-success btn-sm" onclick="return confirm('Paid Invoice <?=$value['invoice_number']?> ?')"><i class="fa fa-check"></i> Paid</a> 
                      <?php } ?>
                    </td> -->
                </tr>
              <?php
