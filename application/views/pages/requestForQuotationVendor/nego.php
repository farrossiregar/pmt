               ...
               <?php
                  foreach($material as $key => $i):
                     ...
                     echo "<tr>";
                        ...
                        $nego = $this->db->get_where('quotation_order_vendor_material', ['material_id' => $i['material_id'], 'quotation_order_vendor_id' => $quotation_order_vendor_id])->row_array();
                        ...
                        echo '<input type="hidden" class="price_nego_input" name="price_nego['. $i['id'] .']" value="'. $row['price_nego'] .'" />';
                        echo $i['qty'] .'</td>';
                        echo '<td class="sub_total">'. format_idr(($i['qty'] * $row['sales_price'])) .'</td>';
                        echo '<td style="background: #ffff97;"><input type="text" class="form-control nego" value="'. @$nego['persen_nego'] .'" name="persen_nego['. $i['id'] .']" /></td>';
                        echo '<td style="background: #ffff97;" class="price_nego">'. @format_idr($nego['price_nego']) .'</td>';
   ...
   $(".nego").each(function(){
      $(this).on('input', function(){
         ...
         if($(this).val() == "") 
         {
            ...
            $(this).parent().parent().find(".price_nego_input").val(0);
         } 
         else
         {
            ...
            $(this).parent().parent().find(".price_nego_input").val(price_nego * qty);
